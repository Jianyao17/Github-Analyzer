...
<template>
  <AuthLayout title="Welcome back" subtitle="Sign in to your account">
    <UForm :state="state" @submit="onSubmit" class="space-y-5">
      <UAlert v-if="error" color="red" variant="subtle" icon="i-lucide-alert-circle" :title="error" />
    ...
    <p class="mt-8 text-center text-sm text-gray-500 dark:text-gray-400">
      ...
      <RouterLink :to="{ name: 'public.register' }"
        class="font-bold text-primary-600 dark:text-primary-400 hover:underline ml-1">
        Create one
      </RouterLink>
    </p>
  </AuthLayout>
