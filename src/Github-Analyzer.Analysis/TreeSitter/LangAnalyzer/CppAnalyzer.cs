...
public class CppAnalyzer : BaseTreeSitterAnalyzer
{
    ...
    protected override void ExtractDeclarations(CodebaseFileContent file, Node rootNode, CodeGraph graph)
    {
        ...
        // Extract Namespaces
        using var nsQuery = new Query(GetTreeSitterLanguage(), "(namespace_definition name: (namespace_identifier) @namespace)");
        ...
        // Extract Classes
        using var classQuery = new Query(GetTreeSitterLanguage(), "(class_specifier name: (type_identifier) @class)");
    ...
    protected override void ExtractUsages(CodebaseFileContent file, Node rootNode, CodeGraph graph)
    {
        ...
        using var callQuery = new Query(GetTreeSitterLanguage(), @"
            ...
            (call_expression function: (field_expression field: (field_identifier) @call.field))
        ");
